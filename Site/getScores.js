...
async function inserirMapa(mapa) {
    ...
    if (mapas.length == 0) {
        downloadImage(`https://cdn.scoresaber.com/covers/${mapa.versions[0].hash.toUpperCase()}.png`, path.resolve(__dirname, 'public', 'assets', 'img', 'mapImg', `${mapa.versions[0].hash.toLowerCase()}.png`));
        let resp = await executar(
